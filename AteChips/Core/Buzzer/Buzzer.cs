...
public partial class Buzzer : IBuzzer, ISettingsChangedNotifier
{
    ...
    public Buzzer(CrystalTimer timer, BuzzerSettings buzzerSettings)
    {
    ...
    public void Reset()
    {
        _normalizationFactor = null;    // Reset normalization factor to force recalculation
        _totalSamplesGenerated = 0;     // Reset sample count to avoid artifacts
        _currentAmplitude = 0f;         // Reset amplitude to avoid clicks
        _phase = 0f;                    // Reset phase to avoid clicks
    }
    ...
    public int GetSamples(float[] buffer, int offset, int count)
    {
        ...
        bool makingSound = (_timer.SoundTimer > 0 && !_buzzerSettings.IsMuted) || TestTone;

        float frequency = _buzzerSettings.Pitch;
        float phaseIncrement = TAU * frequency / SampleRate;

        float targetAmplitude = makingSound ? _buzzerSettings.Volume : 0f;
    ...
    {
        return _buzzerSettings.Waveform switch
        {
    ...
    float PulseWave(float phase) => (phase % 1f) < (_buzzerSettings.PulseDutyCycle / TAU) ? 1f : -1f;
    ...
    float RoundedSquareWave(float phase) => MathF.Tanh(MathF.Sin(phase * TAU) * _buzzerSettings.RoundedSharpness);
    float StaircaseWave(float phase) => (_buzzerSettings.StairSteps <= 1) ? 0f : ((2f * (int)(MathF.Min((phase % 1f), 0.99999994f) * _buzzerSettings.StairSteps) + 1f - _buzzerSettings.StairSteps) / (_buzzerSettings.StairSteps - 1f));
    float ChipTuneLead(float phase) => (0.6f * MathF.Sign(MathF.Sin(phase * TAU))) + (0.4f * (2f * MathF.Abs(2f * (phase % 1f) - 1f) - 1f));
    float StaticBuzzWave(float phase) => 0.7f * ((phase % 1f) < (_buzzerSettings.PulseDutyCycle /TAU) ? 1f : -1f) + 0.3f * (NextFloat() * 2f - 1f);
    ...
    float RetroLaserWave(float phase) =>  MathF.Sin(TAU * phase) * 0.8f - 0.2f + ((phase % 1f) < (_buzzerSettings.PulseDutyCycle / TAU) ? 0.4f : 0f);
    float SolarRampWave(float phase) => MathF.Pow(phase % 1f, 2.5f) * 2f - 1f;
    float MorphPulseWave(float phase) => MathF.Sin(phase) + ((((phase %1f)  < (_buzzerSettings.PulseDutyCycle /TAU)) ? 1f : -1f) - MathF.Sin(phase)) * 0.4f;
